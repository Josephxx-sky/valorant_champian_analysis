...
def load_multi_year_data():
    ...
    # 尝试加载2024年数据
    if os.path.exists("data/2024/processed/all_players.csv"):
        try:
            players_2024 = pd.read_csv("data/2024/processed/all_players.csv")
            data_2024 = preprocess_data(players_2024, '2024')
        except Exception as e:
            st.warning(f"2024年数据加载失败: {str(e)}")
    
    # 尝试加载2025年数据
    if os.path.exists("data/2025/processed/all_players.csv"):
        try:
            players_2025 = pd.read_csv("data/2025/processed/all_players.csv")
            data_2025 = preprocess_data(players_2025, '2025')
        except Exception as e:
            st.warning(f"2025年数据加载失败: {str(e)}")
    
    # 尝试加载旧版数据作为2025年数据
    if data_2025 is None and os.path.exists("data/processed/all_players.csv"):
        try:
            players_2025 = pd.read_csv("data/processed/all_players.csv")
            data_2025 = preprocess_data(players_2025, '2025')
        except Exception as e:
